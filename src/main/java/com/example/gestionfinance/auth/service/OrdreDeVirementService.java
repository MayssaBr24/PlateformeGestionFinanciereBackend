...
@Service
public class OrdreDeVirementService {
    @Autowired
    private EnseignantPermanentRepository enseignantRepo;

    @Autowired
    private EnseignantPermanentService enseignantPermanentService;

    public byte[] genererOrdreDeVirement(LocalDate date, List<Long> enseignantIds) throws Exception {
        ...
        try {
            ...
            // Filtrer les enseignants sélectionnés
            List<EnseignantPermanent> enseignants = enseignantRepo.findAllById(enseignantIds).stream()
                    .filter(e -> e.getCompteBancaire() != null && !e.getCompteBancaire().trim().isEmpty()
                            && "virement bancaire".equalsIgnoreCase(e.getType()))
            ...
            Paragraph titre = new Paragraph("Ordre de Virement – " + moisEtAnnee, fontTitre);
            ...
            document.add(essatInfo);            essatInfo.setSpacingAfter(15f);
            ...
            for (EnseignantPermanent e : enseignants) {
                enseignantPermanentService.payerSalaire(e.getId());
                e = enseignantRepo.findById(e.getId()).orElse(e);
                Double salaire = e.getSalaireBrut();
                ...
                table.addCell((e.getPrenom() != null ? e.getPrenom() : "") + " " + (e.getNom() != null ? e.getNom() : ""));
                table.addCell(e.getBanque() != null ? e.getBanque() : "—");
                table.addCell("Salaire " + moisEtAnnee);
                table.addCell(e.getCompteBancaire());
            }

            document.add(table);

            // Espace après la table
            ...
            totalParagraph.setAlignment(Element.ALIGN_LEFT);  // Changé de ALIGN_CENTER à ALIGN_LEFT
            totalParagraph.setSpacingAfter(5f);  // Espacement réduit entre le montant et la note
            document.add(totalParagraph);

// Note sur les exemplaires alignée à gauche
            Paragraph noteExemplaires = new Paragraph("*Cet ordre est établi en deux exemplaires (un pour la banque et un pour l'ESSAT)");
            noteExemplaires.setAlignment(Element.ALIGN_LEFT);  // Changé de ALIGN_CENTER à ALIGN_LEFT
            noteExemplaires.setSpacingAfter(30f);
            document.add(noteExemplaires);
